...
$start = isset($_REQUEST['page']) ? $page * $kmess - $kmess : (isset($_GET['start']) ? abs(intval($_GET['start'])) : 0);
 ...
 $sql = mysqli_query($kun->connect_db(), "SELECT * FROM `giftcode` WHERE `status`='true' ORDER BY `time` DESC LIMIT $start, $kmess");
 $sql2 = mysqli_query($kun->connect_db(), "SELECT * FROM `giftcode` WHERE `status`='false' ORDER BY `time` DESC LIMIT $start, $kmess");
 $tong1 = mysqli_num_rows(mysqli_query($kun->connect_db(), "SELECT * FROM `giftcode` WHERE `status`='true'"));
 $tong2 = mysqli_num_rows(mysqli_query($kun->connect_db(), "SELECT * FROM `giftcode` WHERE `status`='false'"));
?>
...
$expire_time = $time + $expire;
    ...
    if($giftcode && $percent && $action && $expire) {
        ...
        mysqli_query($kun->connect_db(), "INSERT INTO `giftcode` (`username`, `action`, `giftcode`, `percent`, `status`, `expire`, `time`) VALUES (NULL, '".$action."', '".$giftcode."', '".$percent."', 'true', '".$expire_time."', '".time()."')");
...
<?php
if ($tong1 > $kmess){
    echo '<center>' . $kun->phantrang('/admin/giftcode/', $start, $tong1, $kmess) . '</center>';
}
?>
