...
<div class="col-12">
	<div class="card">
		<div class="card-body">
			...
			<div class="text-right"><a style="color: #fff;" href="?modun=vongquay_kimcuong&act=add" type="button"
					class="btn waves-effect waves-light btn-rounded btn-info">Thêm Vòng Quay</a></div>
		</div>
						...
						<tr>
							...
							<td>
								<div class="switch">
									...
									<label class="cl-switch cl-switch-large">
										<input onclick="options(<?php echo $row['id']; ?>, 'vq_<?php echo $row['id']; ?>')"
											id="vq_<?php echo $row['id']; ?>" type="checkbox" <?php echo $check; ?>>
							...
							<td>
								<a href="?modun=vongquay_kimcuong&act=edit&id=<?php echo $row['id']; ?>">
									<button type="button" class="btn btn-info btn-outline btn-xs m-r-5 tooltip-info"><i
											class="fa fa-edit"></i></button>
								</a>
								<button onclick="del(<?php echo $row['id']; ?>, '<?php echo $row['name']; ?>')" type="button"
									class="btn btn-danger btn-outline btn-xs m-r-5 tooltip-danger"><i
...
<script type="text/javascript">
	...
	function options(id, element) {
		...
		if (checkbox.prop("checked") == true) {
			$.ajax({
				url: '/shopvip/admin/model/vongquaykimcuong/status_vongquay.php',
				type: 'POST',
				dataType: 'text',
				data: {
					id: id,
