...
if($row['giatien'] > $user['money']) {
    die("<script>swal('Lỗi', 'Bạn Không Đủ Tiền để mua Hòm Kim Cương!', 'error');</script>");
}



if(!$row['id']) {
    die("<script>swal('Lỗi', 'Hòm Kim Cương Không Còn Tồn Tại!', 'error');</script>");
}
    ...
    mysqli_query($kun->connect_db(), "INSERT INTO `user_history_system` (`username`, `action`, `action_name`, `sotien`, `mota`, `time`) VALUES ('".$user['username']."', 'Hòm Kim Cương', 'Hòm Kim Cương', '-".number_format($row['giatien'])."đ', 'Nhận được ".$row['kimcuong']." Kim Cương', '".time()."')");
    	// Update Tiền User
